#!/usr/bin/env python3
"""
아시아 시장 포함 종합 경제 데이터 모니터링 시스템 (수정됨)
...
class ComprehensiveEconomicMonitorWithAsia:
    ...
    async def collect_market_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # 아시아 시장 데이터 추가 수집 (올바른 메서드 이름 사용)
            try:
                self.logger.info("🌏 아시아 시장 데이터 수집 중...")
                asian_data = await asyncio.to_thread(
                    self.asian_collector.get_comprehensive_asian_data
                )
                ...
                if asian_data and 'indices' in asian_data:
                    asian_count = len(asian_data['indices'])
                    self.logger.info(f"✅ 아시아 시장 데이터 수집 완료: {asian_count} 시장")
                else:
                    self.logger.info("✅ 아시아 시장 데이터 수집 완료")
            ...
            if self.alpha_collector:
                try:
                    # 올바른 메서드 이름 확인 후 사용
                    if hasattr(self.alpha_collector, 'collect_comprehensive_intelligence_data'):
        ...
        try:
            ...
            comprehensive_data = {
                ...
                'summary': {
                    ...
                    'asian_markets': len(self.asian_market_data.get('indices', {})) if self.asian_market_data else 0,
            ...
            self.logger.info(f"📊 수집 결과: 시장 {len(market_data.get('us_stocks', {}))}개, "
                           f"아시아 {len(self.asian_market_data.get('indices', {})) if self.asian_market_data else 0}개, "
...
async def main():
    ...
    print(f"🌏 아시아 시장: {len(result.get('asian_market_data', {}).get('indices', {})) if result.get('asian_market_data') else 0}개 시장")
